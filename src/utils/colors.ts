...
function colorDistance(color1: {r: number, g: number, b: number}, color2: {r: number, g: number, b: number}) {
  const oklab1 = rgbToOklab(color1.r, color1.g, color1.b);
  const oklab2 = rgbToOklab(color2.r, color2.g, color2.b);
  
  return Math.sqrt(
    Math.pow(oklab1.L - oklab2.L, 2) +
    Math.pow(oklab1.a - oklab2.a, 2) +
    Math.pow(oklab1.b - oklab2.b, 2)
  );
}
...
export function findClosestTailwindColor(hex: string): TailwindColorMatch | null {
  const rgb = hexToRgb(hex);
  if (!rgb) return null;

  let closestColor: TailwindColorMatch = { name: '', shade: '', hex: '', distance: Infinity };
    ...
    for (const [shade, shadeHex] of Object.entries(shades)) {
      ...
      if (shadeRgb) {
        ...
        if (distance < closestColor.distance) {
          closestColor = { name, shade, hex: shadeHex, distance };
        }
      }
    }
  }
